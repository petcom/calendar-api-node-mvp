...
DEST_STORAGE="$DEST_BASE/storage"
...
echo "==> Ensuring directory structure ($DEST_APP, $DEST_LOGS, $DEST_RUN, $DEST_STORAGE)"
sudo mkdir -p "$DEST_APP" "$DEST_LOGS" "$DEST_RUN" "$DEST_STORAGE"

### 3) Rsync relevant files into app/
#   - no delete by default (matches your current behavior)
#   - sensible excludes tailored to this repo
...
### 4) Ownership (entire /srv/calendar-api tree)
